...
# -------------------------
st.set_page_config(page_title="SoM GenAI Learning Interests & Readiness Survey", page_icon="📝", layout="centered")
...
XLSX_PATH = Path("SoM_GenAI_Survey.xlsx")
...
# Utility functions
# -------------------------
def append_row_to_files(row_dict: dict):
    # Ensure all keys in schema
    data = {c: row_dict.get(c, "") for c in COLUMNS}
    df_new = pd.DataFrame([data])

    # CSV append or create
    if CSV_PATH.exists():
        df_csv = pd.read_csv(CSV_PATH)
        df_out = pd.concat([df_csv, df_new], ignore_index=True)
    else:
        df_out = df_new.copy()
    df_out.to_csv(CSV_PATH, index=False)

    # Excel append or create
    if XLSX_PATH.exists():
        try:
            df_xlsx = pd.read_excel(XLSX_PATH)
            df_out_xlsx = pd.concat([df_xlsx, df_new], ignore_index=True)
        except Exception:
            # If schema mismatch or empty, start fresh with columns
            df_out_xlsx = df_new.copy()
    else:
        df_out_xlsx = df_new.copy()
    with pd.ExcelWriter(XLSX_PATH, engine="openpyxl", mode="w") as writer:
        df_out_xlsx.to_excel(writer, index=False, sheet_name="Responses")
...
def multi_select_with_limit(label, options, limit, key):
    ...
    if len(selected) > limit:
        ...
        # Trim to first 'limit' items
        selected = selected[:limit]
        # Force session_state to trimmed value
...
with col1:
    ...
    st.download_button(
        ...
        file_name="SoM_GenAI_Survey.csv",
        mime="text/csv",
    )
with col2:
    xlsx_buf = io.BytesIO()
    with pd.ExcelWriter(xlsx_buf, engine="openpyxl") as writer:
        current_df.to_excel(writer, index=False, sheet_name="Responses")
    st.download_button(
        "Download Excel",
        data=xlsx_buf.getvalue(),
        file_name="SoM_GenAI_Survey.xlsx",
...
with st.expander("Deployment tips"):
    st.markdown(
        ...
        "- Host on Streamlit Community Cloud (public) or inside your org’s Streamlit server.\n"
        "- Files are saved to the app’s working directory. For shared storage, point to a network path or replace writers with SharePoint/OneDrive APIs or a database."
    )
